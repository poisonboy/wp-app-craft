...
function send_email_code($request)
{
    ...
    if ($is_email_sent) {
        ...
        if (!function_exists('mail')) {
            return new WP_REST_Response(array('status' => 'error', 'message' => __('Server does not support email sending', 'wp-app-craft')), 400);
        } else {
...
function common_register_logic($username, $password, $email, $mobile, $nickname, $inviter_id)
{
    ...
    if (!is_wp_error($user_id)) {
        ...
        appcraft_manage_points($user_id, $initial_points, __('New user registration', 'wp-app-craft'));
...
function appcraft_register_user($request)
{
    ...
    if (is_null($username) || is_null($password) || is_null($email) || is_null($nickname) || is_null($code)) {
        ...
        return new WP_REST_Response(['status' => 'error', 'message' => __('Incomplete parameters', 'wp-app-craft')], 400);
    } 
    if (empty($username) || strlen($username) < 3 || strlen($username) > 20) {
        return new WP_REST_Response(['status' => 'error', 'message' => __('Username length must be between 3-20 characters', 'wp-app-craft')], 400);
    }
...
function appcraft_give_inviter_points($user_id, $inviter_id)
{
    ...
    // 使用通用函数来添加积分
    appcraft_manage_points($inviter_id, $points_for_invitation, sprintf(__('Invited new user with ID %s', 'wp-app-craft'), $user_id));
