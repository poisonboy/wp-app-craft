...
function build_comment_tree($comments) {
    ...
    foreach ($comments as $comment) {
        ...
        $comments_by_id[$comment->comment_ID] = array(
            ...
            'date' => time_elapsed_string($comment->comment_date),
...
function create_comment_response($comments) {
    return array(
        'code' => '200',
        'message' => 'success',
        'data' => array(
...
function appcraft_get_comments($data) {
    if (isset($data['post_id']) && empty($data['post_id'])) {
        return create_comment_response(array());
    }
    ...
    return create_comment_response($comment_tree);
}
