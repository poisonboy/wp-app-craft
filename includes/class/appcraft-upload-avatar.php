...
function appcraft_upload_user_avatar(WP_REST_Request $request)
{
  ...
  if ($upload_count <= 0) {
    return new WP_REST_Response(['status' => 'error', 'message' => __('Avatar upload limit reached', 'app-craft')], 403);
  }
  $upload_count--;
  carbon_set_user_meta($user_id, 'appcraft_upload_count', $upload_count);

  if (!isset($_FILES['avatar'])) {
    return wp_send_json_error(__('Avatar not provided', 'app-craft'));
  }
  ...
  if (!in_array($file_type, $allowed_types)) {
    return wp_send_json_error(__('File type not allowed', 'app-craft'));
  }

  if ($avatar['size'] > 2 * 1024 * 1024) {
    return wp_send_json_error(__('File size exceeds limit', 'app-craft'));
  }
  ...
  if ($movefile && !isset($movefile['error'])) {
    $response_data = array(
      "code" => "200",
      "message" => __("Avatar uploaded successfully", 'app-craft'),
...
function validateAndGetAvatar($data)
{
  ...
  if (!isset($avatar['name'], $avatar['type'])) {
    return wp_send_json_error(__('Incorrect parameters', 'app-craft'));
  }
