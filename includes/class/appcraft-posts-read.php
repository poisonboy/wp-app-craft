...
function appcraft_pay_for_article($request)
{
    ...
    // 扣除积分
    $result = appcraft_manage_points($user_id, $pay_points, sprintf(__('Paid content for reading article with ID %s', 'wp-app-craft'), $article_id), 'subtract', AC_TYPE_PAY_CONTENT, $article_id);
...
function appcraft_reward_for_article($request)
{
    ...
    $reward_points = appcraft_get_random_points_from_range($reward_range);
    $has_earned = get_user_meta($user_id, '_appcraft_read_earned_' . $article_id, true)  ==  'earned';

    if ($has_earned) {
        return ['success' => false, 'message' => __('You have already earned the reward for this article.', 'wp-app-craft')];
    }
    // 增加积分
    $result = appcraft_manage_points($user_id, $reward_points, sprintf(__('Earned reward for reading article with ID %s', 'wp-app-craft'), $article_id), 'add', AC_TYPE_ARTICLE_READ, $article_id);
    if (is_wp_error($result)) {
        return $result;
    }
...
function appcraft_get_random_points_from_range($range)
{
