...
function appcraft_signin_api($request) {
   $user_id = verify_user_token($request);
    ...
    return new WP_REST_Response(['message' => __('Check-in successful', 'app-craft')], 200);
...
function appcraft_user_sign_in($user_id) {
    ...
    if ($last_signin_date === $today) {
        return new WP_Error('already_signed_in', __('Already checked in today', 'app-craft'), array('status' => 400));
    }
    ...
    // 管理积分
    $result = appcraft_manage_points($user_id, $signin_points, __('User check-in', 'app-craft'));
    ...
    if ($consecutive_days == 7) {
        appcraft_manage_points($user_id, $seven_day_bonus, __('7-day consecutive check-in reward', 'app-craft'));
    ...
    return array('message' => __('Check-in successful', 'app-craft'));
}
