...
function appcraft_update_password(WP_REST_Request $request)
{
    ...
    if (strlen($new_password) < 8) {
        return new WP_REST_Response(['status' => 'error', 'message' => __('Password must be at least 8 characters long', 'app-craft')], 400);
    }
    ...
    if (!preg_match('/[A-Za-z]/', $new_password) || !preg_match('/[0-9]/', $new_password)) {
        return new WP_REST_Response(['status' => 'error', 'message' => __('Password must contain at least one letter and one number', 'app-craft')], 400);
    }

    // Update password
    $update_status = wp_set_password($new_password, $user_id);
    if ($update_status === false) {
        return new WP_REST_Response(['status' => 'error', 'message' => __('Password update failed', 'app-craft')], 500);
    }

    return new WP_REST_Response(['status' => 'success', 'message' => __('Password updated', 'app-craft')], 200);
}
...
function appcraft_send_delete_confirmation_email(WP_REST_Request $request)
{
    ...
    // Create API link
    $api_url = rest_url('app-craft/v1/confirm-delete');
    ...
    if ($last_sent && (time() - $last_sent < 900)) {
        return new WP_REST_Response(['status' => 'error', 'message' => __('Please wait 15 minutes before trying again', 'app-craft')], 429);
    }

    // Send confirmation email
    $email = get_the_author_meta('user_email', $user_id);
    $message = __("Please reply using the email associated with your account, specifying the UID of the account to be deleted and the reason. We will lock your account upon receiving the email and permanently delete all information after one week. \n\nNote: This action is irreversible. If you did not request account deletion, please ignore this email.", 'app-craft');
    $is_email_sent = wp_mail($email, __('Confirm Account Deletion', 'app-craft'), $message);

    // Respond based on the status of the email sent
    if ($is_email_sent) {
        set_transient("email_last_sent_" . $user_id, time(), 300);
        return new WP_REST_Response(['status' => 'success', 'message' => __('Confirmation email sent, please check your inbox.', 'app-craft')], 200);
    } else {
        return new WP_REST_Response(['status' => 'error', 'message' => __('Failed to send email, please try again later.', 'app-craft')], 500);
    }
}
...
function appcraft_confirm_and_delete_account(WP_REST_Request $request)
{
    ...
    if ($expected_token !== $token || time() - $timestamp > 3600) {
        // Token 无效或过期
        return new WP_REST_Response(['status' => 'error', 'message' => __('Invalid or expired token', 'app-craft')], 400);
    }
    ...
    return new WP_REST_Response(['status' => 'success', 'message' => __('Account deleted', 'app-craft')], 200);
}
